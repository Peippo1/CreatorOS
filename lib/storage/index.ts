...
function normalizeExperiment(row: Record<string, unknown>): ContentExperiment { return { ...row, sourceDocumentId: row.source_document_id, sourceReference: row.source_reference ?? "", sourceItemKey: row.source_item_key ?? "", audienceSegment: row.audience_segment, audienceProblem: row.audience_problem, intendedOutcome: row.intended_outcome, plannedPublishDate: row.planned_publish_date, variantLabel: row.variant_label, createdAt: row.created_at, updatedAt: row.updated_at } as ContentExperiment; }
