...
class SupabaseStore implements Store {
  ...
  async listSources(userId: string) { const { data, error } = await this.client.from("source_documents").select("id,title,type,content,created_at").eq("user_id", userId).order("created_at", { ascending: false }); if (error) throw error; return (data ?? []).map((row) => ({ id: row.id, title: row.title, type: row.type, content: row.content, createdAt: row.created_at } as SourceDocument)); }
  async addSource(userId: string, source: Omit<SourceDocument, "id" | "createdAt">) { const { data, error } = await this.client.from("source_documents").insert({ user_id: userId, ...source }).select("id,title,type,content,created_at").single(); if (error) throw error; return { id: data.id, title: data.title, type: data.type, content: data.content, createdAt: data.created_at } as SourceDocument; }
