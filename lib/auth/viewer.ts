...
export async function getViewer(): Promise<Viewer | null> {
  if (!isSupabaseConfigured()) {
    // A missing production auth configuration must never silently expose the
    // demo workspace or paid generation endpoint.
    if (process.env.NODE_ENV === "production") return null;
    return { id: "demo-user", email: "demo@creatoros.local", demo: true };
  }
